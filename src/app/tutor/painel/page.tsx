...
export default function TutorDashboardPage() {
  ...
  // Como as notas/faltas reias virão depois, por padrão vamos dizer que estão todos "Normal" se acabaram de ser criados.
  const emRisco = 0;
  const emAtencao = 0;
              ...
              alunos.slice(0, 3).map(aluno => (
                <div key={aluno.id} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{aluno.turma} • RA: {aluno.matricula}</p>
                  </div>
